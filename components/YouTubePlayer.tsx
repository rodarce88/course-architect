...
const YouTubePlayer = forwardRef<YTPlayerHandle, { videoId: string; startAt?: number; onTimeUpdate?: (t: number) => void }>(({ videoId, startAt, onTimeUpdate }, ref) => {
  ...
  useEffect(() => {
    ...
    const init = async () => {
      ...
      playerRef.current = new window.YT.Player(el, {
        ...
        playerVars: { autoplay: 1, modestbranding: 1, rel: 0, start: startAt ? Math.floor(startAt) : undefined },
        events: {
          onReady: () => {
            if (dead) return
